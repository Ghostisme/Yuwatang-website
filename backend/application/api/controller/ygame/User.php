...
class User extends Api
{
    ...
    public function cert(){
        ...
        $count = $model->where('user_id',$this->auth->id)->count();
        if($page){
            $data = $model->with('project')->where('user_id',$this->auth->id)->page($page,$limit)->order("id desc")->select();

        }else{
            
            $data = $model->with('project')->where('user_id',$this->auth->id)->order("id desc")->select();
        }
//        return $data;
...
//        $this->success('s',$data);
        foreach($data['data'] as &$v){
            $v['cert_url'] = $this->request->domain()."/".$v['cert_url'];

            $v['project']['image'] = $this->request->domain()."/".$v['project']['image'];
            $v['project']['createtime'] = date('Y-m-d H:i:s',    $v['project']['createtime']);
        }
        $this->success('请求成功', $data);
    }
}
